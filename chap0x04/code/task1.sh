...
function Help() {
    ...
    echo "-w font_size font_color text  对图片批量添加自定义文本水印"
...
function AddCustomWatermark {
    for i in *;do
        ...
        convert "${i}" -pointsize "$1" -fill "$2" -stroke black -draw "text 250,75 '$3'" "${i}"
        echo "${i} 自定义文本水印 $3 已添加完成"
    done
}
